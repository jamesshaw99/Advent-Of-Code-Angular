...
describe('2024 day12', () => {
    ...
    describe('findAllRegions', () => {
        it('should find all regions in the grid', () => {
            ...
            expect(regions).toContainEqual(expect.objectContaining({ plantType: 'R', area: 12 }));
            expect(regions).toContainEqual(expect.objectContaining({ plantType: 'I', area: 4 }));
            expect(regions).toContainEqual(expect.objectContaining({ plantType: 'C', area: 14 }));
            expect(regions).toContainEqual(expect.objectContaining({ plantType: 'F', area: 10 }));
            expect(regions).toContainEqual(expect.objectContaining({ plantType: 'V', area: 13 }));
            expect(regions).toContainEqual(expect.objectContaining({ plantType: 'J', area: 11 }));
            expect(regions).toContainEqual(expect.objectContaining({ plantType: 'C', area: 1 }));
            expect(regions).toContainEqual(expect.objectContaining({ plantType: 'E', area: 13 }));
            expect(regions).toContainEqual(expect.objectContaining({ plantType: 'I', area: 14 }));
            expect(regions).toContainEqual(expect.objectContaining({ plantType: 'M', area: 5 }));
            expect(regions).toContainEqual(expect.objectContaining({ plantType: 'S', area: 3 }));
        });
    });
